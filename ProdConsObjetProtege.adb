...
--sauvegarder ce fichier sous le nom de: ProdConsObjetProtege.adb, et pour compiler lancer: gnatmake ProdConsObjetProtege.adb -o Prodconsi
...
entry produire(Mess : IN Integer) -- Y'a plus le accept on met directe dans  le entry
when (cpt < n) is
begin
tampon(tete) := Mess;
tete := (tete + 1) mod n;
